...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr  7 16:45:40 2020
...
def columnCost():
    # This function calculate the affected columns
    col1temp = [col[col1] for col in puzzle]
    col2temp = [col[col2] for col in puzzle]
    totalCost = 0
    col1Cost = 0
    col2Cost = 0
    for ele in range(n):
        if(ele not in col1temp):
            col1Cost += 1
        if(ele not in col2temp):
            col2Cost += 1
    totalCost = col1Cost + col2Cost
    return totalCost
...
def swap(Z):
    ...
    # It calculates the cost of the choicen columns
    colsCost = columnCost()
    print("the cost of two selected columns(before swapping) are: " + str(colsCost))
...
if(numberOfGivens!=0):
    ...
    for i in range(time):
        ...
        print("the cost of two selected columns (after swapping) are: " + str(columnCost()))
        if (columnCost()< colsCost):
            colsCost = columnCost()
        else:
        ...
        printSquare(puzzle)
    print("-------------------------------------------------------------")
    print("The total cost of the matrix after all of the iteration is: " + str(cost(puzzle)))
